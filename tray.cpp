...
void Tray::updateApplicationState()
{
    if (partialUpgrade() || (isSystemCriticallyOutOfDate() && settings.contains("timestamps/systemupdate-alert"))) {
        trayicon->setIconByName("vampire-system-maintenance-alert");
        trayicon->setStatus(KStatusNotifierItem::NeedsAttention);
    } else {
        trayicon->setIconByName("vampire-system-maintenance");
    ...
    : QMainWindow(parent)
{
    ...
    QAction* settingsAction = new QAction(
        QIcon::fromTheme("vampire-system-maintenance"),
        "Settings",
        menu);
...
void Tray::updateKeyring(bool keyring, bool hotfixes)
{
        ...
        do {
            ...
            if (reply == QMessageBox::Yes) {
                ...
                process->start("systemctl", QStringList() << "start"
                                                          << "vampire-system-maintenance@keyring-hotfixes.service");
                return;
    ...
    if (keyring) {
        trayicon->showMessage("Vampire System Maintenance", "Updating keyrings in the background...", "vampire-system-maintenance");
        ...
        process->start("systemctl", QStringList() << "start"
                                                  << "vampire-system-maintenance@keyring.service");
    }
}
...
void Tray::onKeyringsInstalled(int exitcode, QProcess::ExitStatus status, QProcess* process, bool hotfix)
{
    if (status == QProcess::ExitStatus::NormalExit && exitcode == 0) {
        trayicon->showMessage("Vampire System Maintenance", hotfix ? "Hotfix successfully applied!" : "Keyrings successfully updated!", "vampire-system-maintenance");
    } else {
        trayicon->showMessage("Vampire System Maintenance", hotfix ? "Hotfix failed!" : "Keyring update failed!", "vampire-system-maintenance");
    }
    busy = false;
    process->deleteLater();
